	...
	<br><br><br><br>
	  <div class="container">
	    <div class="row">
	    	<div class="col-md-8">
	    		<br><br><br><br>
	    		<h1 class="display-4">Tiket Kereta</h1>
	    		<p class="lead">Anda dapat melakukan pemesanan tiket dengan mudah</p>
	    	</div>
	    	<div class="col-md-4">
	    		<form action="<?= base_url('index.php/cariTiket') ?>" method="POST">
	  		  		...
	  		  		<tbody>
	  		  			...
	  		  			<?php foreach ($tiket as $tk) { ?>
	  		  				<tr>
	  		  					...
	  		  					<td><?php $date = date_create($tk->tgll_berangkat); echo date_format($date, "d-m-y h:i:s"); ?></td>
	  		  					<td><?php $date = date_create($tk->tg_sampai); echo date_format($date, "d-m-y h:i:s"); ?></td>
	  		  					<td>
	  		  						<a class="btn btn-success" href="<?= base_url('index.php/pesan/'.$tk->id.'?p='.$penumpang) ?>">Pesan</a>
	  		  					</td>
	  		  				</tr>
	  		  			<?php } ?>
